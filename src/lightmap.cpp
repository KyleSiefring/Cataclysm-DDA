    ...
    const float numerator = 1.0f, const int row = 1 )
{
    ...
    std::deque<VisibleRegion> last_stack;
    std::deque<VisibleRegion> stack;
    last_stack.push_back( { 0, 1, 0, 1, LIGHT_TRANSPARENCY_OPEN_AIR, 0 } );
    ...
    for( int distance = row; distance <= radius; distance++ ) {
        ...
        while ( !last_stack.empty() ) {
            VisibleRegion region = last_stack.back();
            last_stack.pop_back();
            slope start_major = region.start_major;
            slope end_major = region.end_major;
            slope start_minor = region.start_minor;
            slope end_minor = region.end_minor;
            double cumulative_transparency = region.cumulative_transparency;
